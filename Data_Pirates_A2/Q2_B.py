...
def paging(d):
    for i in range(Cardinality[d]):
        data.where(data[Dims[d]]==dist_values[d][i]).to_csv("./"+dist_values[d][i]+".csv")
...
for i in range(100, 5000, 500):
    ...
    for d in range(numDims):
        ...
        for r in range(Cardinality[d]):
            # input=data.where(data[Dims[d]]==dist_values[d][i])
            input=data.loc[data[Dims[d]]==dist_values[d][r]]
            BUC(input, d, i)
